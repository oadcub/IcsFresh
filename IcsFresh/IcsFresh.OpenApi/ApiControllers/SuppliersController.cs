...
namespace IcsFresh.OpenApi.ApiControllers
{
    ...
    public class SuppliersController : ApiControllerBase
    {
        ...
        public async Task<IHttpActionResult> Fetch()
        {
            try
            {
                result.Datas.Data1 = await db.Suppliers.OrderBy(x=>x.Name).ToListAsync();
                return Json(result);
            }
        ...
        [Route("Insert")]
        [ResponseType(typeof(JsonResultWrapper))]
        public async Task<IHttpActionResult> Insert(Supplier viewModel)
        {
            try
            {
                db.Suppliers.Add(viewModel);
