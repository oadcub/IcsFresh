...
//    for (i in listKey) {
//        params.List_CoreWebResource.push({ ResourceName: listKey[i] });

//    }
//    var List_CoreWebResource = [];
//    assignCoreWebResource('[res]', List_CoreWebResource, '', '');
//    console.log(params);

//}

////เชคให้รับเฉพาะตัวเลขเท่านั้น onKeyPress="return Check_Key(event);"
...
////onBlur="return Check_Format_Number(this);" check key number&dot only
//function Check_Format_Number(txtMoney) {
...
////onKeyPress="return Check_Key_Decimal(this,event,1)" 
////check key number&dot only and decimal (iDigit Ex 1=2digit, 2=3digit) digit
//function Check_Key_Decimal(txtMoney, e, iDigit) {
//    var Money = txtMoney;
//    var key;
//    if (document.all) {
//        key = window.event.keyCode;
...
//            }
//        } else {
//            if (String(Money.value).split(".").length > iDigit) {
//                if (String(Money.value).split(".")[1].length > iDigit) {
//                    return false;
//                }
//                else {
//                    return true;
...
////onBlur="return Check_Format_Decimal(this, 2);" 
////check key number&dot only and decimal (iDigit Ex 2=2digit, 3=3digit)  digit
//function Check_Format_Decimal(txtMoney, iDigit) {
//    //var Money = document.getElementById(txtMoney.id).value;
//    var Money = txtMoney.value;
//    if (Money != "") {
//        txtMoney.value = numberWithCommas(parseFloat(numberWithCommasValue(Money)).toFixed(iDigit), iDigit);
//    }
//    return true;
//}

//function Check_Format_Decimal_Data(txtMoney, iDigit) {
//    var Money = txtMoney;
//    if (Money != "") {
//        txtMoney = numberWithCommas(parseFloat(numberWithCommasValue(Money)).toFixed(iDigit), iDigit);
//    }
//    return true;
//}

//function numberWithCommas(x, iDigit) {
//    var sDigit = ".00";
//    if (iDigit == 2) {
//        sDigit = ".00";
//    }
...
//        if (!isNaN(x))
//            x = roundToTwo(x, iDigit);
//        var result = x.toString();//.replace(/\B(?=(\d{3})+(?!\d))/g, ",");

//        //var newValue = result;
//        //var splitedValue = newValue.split(".");
//        //var integerPart = splitedValue[0];
//        //var fractionPart = splitedValue[1];
//        //return [integerPart.replace(/\B(?=(\d{3})+(?!\d))/g, ","), fractionPart].join(iDigit);

//        if (result.indexOf('.') == -1) {
//            return result.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + sDigit;
//        }
//        else  //if (result.indexOf('.') + iDigit == result.length) 12345.4 to 12345.40 
//        {
//            var newValue = result;
//            var splitedValue = newValue.split(".");
//            var integerPart = splitedValue[0];
...
//            if (fractionPart.length < iDigit) {              
//                var iItem = 0;
//                for (iItem = fractionPart.length; iItem < iDigit; iItem++) {
...
//function roundToTwo(num, iDigit) {
//    return +(Math.round(num + "e+" + iDigit) + "e-" + iDigit);
//}
